...
switch (_boxType) do
{
	...
	case "airdrop_water":
	{
		...
		[
			// Item type, Item class(es), # of items, # of magazines per weapon
			["itm", ["Exile_Item_PlasticBottleCoffee"], 20]
		];
	};
	case "airdrop_foodandwater":
	{
		_boxItems =
		[
			// Item type, Item class(es), # of items, # of magazines per weapon
	...
	case "airdrop_Med":
	{
		...
		[
			...
			["itm", ["Exile_Item_Defibrillator"], 2]
		];
	};	
};

[_box, _boxItems] call d2k_fnc_processItems;
